...
# Functions
    ...
    matchIDs_multiple = function(genotype_path, snps_data, genotype_type, outdir, maf, freq, freq_mode, script_path, plink_path, plink2_path, imputeMissing){
        ...
        if (imputeMissing == TRUE){
            ...
            matchingsnps_all = rbind.fill(matchingsnps_all, data.frame(chr=missing_snps$CHROM, id=missing_snps$id, na=rep(0, nrow(missing_snps)), pos=missing_snps$POS, ref=missing_snps$OTHER_ALLELE, alt=missing_snps$EFFECT_ALLELE, unique_id=missing_snps$id, allele_match=rep('imputed', nrow(missing_snps))))
        }
    ...
    imputeMissingVariants = function(all_dos, snps_data, matchingsnps_all){
        ...
        # Check if there are missing snps to impute
        if (nrow(missing_snps) >0){
            ...
            return(list(all_dos, missing_snps))
        }
    }
    ...
    reportPhenoStats = function(res_prs, assoc_info, outdir, assoc_mode, assoc_var, assoc_cov, assoc_survival, suffix){
        tryCatch({
             cat(paste0('****** Compiling phenotype report for variable: ', assoc_var, ' ', str_replace_all(suffix, '_', ''), '\n'))
            # Define output file
            ...
            # All individuals
                ...
                if ('SEX' %in% toupper(colnames(combined_data))){
                    ...
                    # PRS stats by sex
                    tmp_sexes = unique(na.omit(combined_data$SEX))
                    for (sex in tmp_sexes){
                        ...
                        iqr_val = paste(round(quantile(res_prs$PRS[combined_data$SEX == sex], probs = c(0.25, 0.75), na.rm = TRUE), 3), collapse = '-')
                        # Write the stats to the file
            ...
            cat(paste0("****** Error in compiling phenotype report. Error message: ", e$message, "\n"))
        })
    }
